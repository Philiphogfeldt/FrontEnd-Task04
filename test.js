Vue.createApp({
  ...
  methods: {
    ...
    async getDepartures() {
      ...
      if (this.departures1.length < 5) {
        const newDepartures = data.DepartureBoard.Departure
          .map((departure) => {
          ...
          .slice(0, 5 - this.departures1.length);

        this.departures1.push(...newDepartures);
      } else if (this.departures2.length < 5) {
        const newDepartures = data.DepartureBoard.Departure
          .map((departure) => {
            const { sname, direction, time, date, rtTime, rtDate } = departure;
          ...
          .sort((a, b) => a.diff - b.diff)
          .slice(0, 5 - this.departures2.length);

        this.departures2.push(...newDepartures);
      } else {
        const newDepartures = data.DepartureBoard.Departure
          .map((departure) => {
            const { sname, direction, time, date, rtTime, rtDate } = departure;
